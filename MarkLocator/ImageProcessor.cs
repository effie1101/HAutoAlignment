...
namespace FC.MarkLocator
{
    public class ProcessImage
    {
        ...
        public bool MarkAlignment(string imgFile, HTuple windowID)
        {
            ...
            //计算相对位移
            this.OutputManager.AlignmentSita = Math.Round(OutputManager.Angle + this.InputManager.CCD2ProbeSita, 5);
            this.OutputManager.AlignmentX = Math.Round(0-this.InputManager.ProbeHeadRotateR * Math.Sin(this.OutputManager.AlignmentSita / 180 * Math.PI) + this.InputManager.CCD2ProbeX + (col / IMG_WIDTH * CCD_W - CCD_W / 2) + this.InputManager.BondpadCenterX, 5);
            this.OutputManager.AlignmentY = Math.Round(this.InputManager.ProbeHeadRotateR * (1 - Math.Cos(this.OutputManager.AlignmentSita / 180 * Math.PI)) + this.InputManager.CCD2ProbeY + (row / IMG_HEIGHT * CCD_H - CCD_H / 2) + this.InputManager.BondpadCenterY, 5);
            ...
            //this.OutputManager.AlignmentX = Math.Round(this.InputManager.BondpadCenterY - this.InputManager.CCD2ProbeY - (row / IMG_HEIGHT * CCD_H - CCD_H / 2) - this.InputManager.CCD2ProbeY - this.InputManager.ProbeHeadRotateR * (1 - Math.Cos(this.OutputManager.AlignmentSita / 180 * Math.PI)), 2);
